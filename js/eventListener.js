    ...
    document.getElementById('world').onmousemove = function (e) {
        ...
        if (currentTool == 'move') {
            if (isGridMouseDown) {
                ...
                grid.displayClear();
                grid.updateOffset(tempGridDx + dx, tempGridDy + dy);
                grid.displayAll();
            }
        }
        else {
            if (CELL_SIZE >= 5) {
                ...
                // INSIDE THE WORLD
                if (isInsideGrid(x, y)) {
                    let cellX = Math.floor((x - grid.dx - grid.marginX) / CELL_SIZE);
                    let cellY = Math.floor((y - grid.dy - grid.marginY) / CELL_SIZE);
                    if (cellX == lastX && cellY == lastY) {
                        ...
                        switch (currentTool) {
                            case 'default':
                                ...
                                grid.hoveredCell.x = cellX;
                                grid.hoveredCell.y = cellY;
                                grid.hoveredCell.isAlive = grid.getState(cellX, cellY);

                                grid.drawCell(cellX, cellY, CELL_HOVER_COLOR);
                                break;
                            case "block":
                            case "bee-hive":
                            case "loaf":
